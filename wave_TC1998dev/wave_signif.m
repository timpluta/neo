...
% fft_theor = (1-lag1^2) ./ (1-2*lag1*cos(freq*2*pi)+lag1^2);  % [Eqn(16)] Red Noise 
lag2 = 0.0005;   % AR-lag varying... to gen pink noise
lag3 = 0.001;   % still varying 

fft_theor1 = (1) ./ (1-lag1)^2 + 2*lag1*(1-cos(freq)*2*pi); % mucking around adding Pink Noise
fft_theor2 = (1) ./ (1-lag2)^2 + 2*lag2*(1-cos(freq)*2*pi); % mucking around adding Pink Noise
fft_theor3 = (1) ./ (1-lag3)^2 + 2*lag3*(1-cos(freq)*2*pi); % mucking around adding Pink Noise
fft_theor = fft_theor1 + fft_theor2 + fft_theor3; % summing
